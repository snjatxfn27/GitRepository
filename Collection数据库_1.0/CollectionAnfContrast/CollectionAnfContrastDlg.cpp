...
CString CCollectionAnfContrastDlg::GetImagePath(CString csImageName, CString csFile)
{
	...
	CString csFullPath(pFileName);
	csTemp.Format("%s%s%s%scurface_%s.b64", csFullPath, IMAGE_ROOTPATH, csFile, IMAGE_PATH, csImageName);
	return csTemp;
}
//获取warning 路径
	...
	try
	{
		...
		if (g_nContrastComputer > 0)
		{
			...
			while (!pRecordset->adoEOF)
			{
				...
				// 				CString csFullPath = GetiniPath();
				// 				CString csSimilarity;
				// 				GetPrivateProfileString(("Computer"), "Camera_Similarity ", (""), csSimilarity.GetBuffer(MAX_PATH), MAX_PATH, csFullPath);
...
UINT ThreadScheduling(LPVOID lparam)
{
	...
	while (bWhile)
	{
		try
		{
			//修改退出while 2017年5月5日10:43:04
			// 			if (g_bIsMOdify)
			// 			{
			// 				bWhile = FALSE;
			// 				for (int i =0; i < nWorkTheradSum; i++)
			// 				{
			// 					SetEvent(g_ThandSchedulingArr[i].HEvent);
			// 				}
			// 				continue;
			// 			}
			...
			while (!pRecordset->adoEOF)
			{
				...
				//如果所开进程都在工作状态休眠2秒
				if (g_ThandWorkState >= g_ThandWorkOnline)
				{
					DT_Log_Error(__FILE__, __LINE__, DTUseLevel[4], 1, "WorkState > WorkOnline");
					Sleep(pWnd->m_nBideTime * 1000);
				}

				//循环变量置为起始位置
				if (nIdex >= nWorkTheradSum)
					nIdex = 0;

				//循环判断在线一体机是否在工作
				for (; nIdex < nWorkTheradSum; nIdex++)
				{
					if (!g_ThandSchedulingArr[nIdex].bWorkState &&	g_ThandSchedulingArr[nIdex].bIsOnline)
					{
						//需要等表修改完做修改
						EnterCriticalSection(&cSECTION);
						g_ThandSchedulingArr[nIdex].bWorkState = TRUE;
						g_ThandSchedulingArr[nIdex].csImgName = (TCHAR *)(_bstr_t)pRecordset->GetFields()->GetItem("id")->Value;
						g_ThandSchedulingArr[nIdex].csImgFile = (TCHAR *)(_bstr_t)pRecordset->GetFields()->GetItem("imgpath")->Value;
						//添加IP字段
						csSql.Format("UPDATE snapshotinfo SET process_status = '4'  , RecognitionIp = '%s' where id = '%s' ",
							g_ThandSchedulingArr[nIdex].csComputerIp, g_ThandSchedulingArr[nIdex].csImgName);
						g_pConnect->Execute(_bstr_t(csSql), NULL, adCmdText);
						LeaveCriticalSection(&cSECTION);

						CString csTemp;
						csTemp.Format("SetEvent_%s_nIdex:%d_nWorkTheradSum:%d", g_ThandSchedulingArr[nIdex].csComputerIp, nIdex, nWorkTheradSum);
						DT_Log_Error(__FILE__, __LINE__, DTUseLevel[4], 1, csTemp);
						...
						if (!SetEvent(g_ThandSchedulingArr[nIdex].HEvent))
						{
							...
							CString csTemp;
							csTemp.Format("SetEvent_%s", g_ThandSchedulingArr[nIdex].csComputerIp);
							DT_Log_Error(__FILE__, __LINE__, DTUseLevel[4], 1, csTemp);
							continue;
						}
						nIdex++;
...
UINT ThreadContrast(LPVOID lparam)
{
	...
	DWORD dReturn;;
	BOOL bWhile = TRUE;
	CString csImgPath, csSql, csTemp;
	int nContrast = 0;
	int nConut = 0;
	char pSession[19];
	...
	while (bWhile)
	{
		try
		{
			...
			if (WAIT_OBJECT_0 == dReturn)
			{
				...
				CString csTemp, csNameID, csImgFile;
				memset(pSession, '\0', 19);

				//需要加锁	
				EnterCriticalSection(&cSECTION);
				g_ThandWorkState++;
				//	g_ThandSchedulingArr[nIdex].bWorkState = TRUE;
				//将结构体内的值赋值到局部变量防止外部修改
				csNameID = g_ThandSchedulingArr[nIdex].csImgName;
				csImgFile = g_ThandSchedulingArr[nIdex].csImgFile;
				//获取照片路径
				csImgPath = pWnd->GetImagePath(csNameID, csImgFile);
				LeaveCriticalSection(&cSECTION);

				csTemp.Format("ThreadContrast_%s_%s_Start", g_ThandSchedulingArr[nIdex].csComputerIp, csNameID);
				DT_Log_Error(__FILE__, __LINE__, DTUseLevel[4], 1, csTemp);
				...
				{
					csSql.Format("UPDATE snapshotinfo SET process_status = '11' where id = '%s' ", csNameID);
					g_pConnect->Execute(_bstr_t(csSql), NULL, adCmdText);
					EnterCriticalSection(&cSECTION);
					g_ThandSchedulingArr[nIdex].bWorkState = FALSE;
					g_ThandWorkState--;
				...
				if (nContrast == 0)//对比成功3
				{
					nWarning = 0;
					//修改抓拍表 标志位
					csSql.Format("UPDATE snapshotinfo SET process_status = '3'  ,warningcount = %d  where id = '%s' ",
						nConut,
						csNameID);
					g_pConnect->Execute(_bstr_t(csSql), NULL, adCmdText);
					//向warning 表写对比到的八项数据
				...
				else //对比失败10
				{
					...
					csSql.Format("UPDATE snapshotinfo SET process_status = '10' where id = '%s' ", csNameID);
					g_pConnect->Execute(_bstr_t(csSql), NULL, adCmdText);
				}
		...
		{
			CString csTemp;
			csTemp.Format("ThreadContrast_%s_%s_catch", g_ThandSchedulingArr[nIdex].csComputerIp, g_ThandSchedulingArr[nIdex].csImgName);
			DT_Log_Error(__FILE__, __LINE__, DTUseLevel[4], 1, csTemp);
		}
	}
