...
int CheckWhite(CString strIP, CString strSession, CString strName, char*chJson, int jsonlen)
{
	...
	CString strSend;
	CString src = L"/api/v1/facedb/white?name=" + strName;
	...
	char *revbuf = new char[8 * 1024];
	memset(revbuf, '\0', 8 * 1024);
	int iCurPos=0;
	while (1)
	{
...
int AddWhite(CString strIP, CString strSession, CString strName, char*chJson, int jsonlen)
{
	...
	strSend.Format(L"%s%s%s%s%s%s%d%s%s%s%s%s%s%s%s",
		L"POST /api/v1/facedb/white HTTP/1.1\n",
		L"Accept: */*\n",
		L"Host: ", strIP, L"\n",
	...
	char * revbuf = new char[8 * 1024];
	memset(revbuf, '\0', 8 * 1024);
	recv(sockClient, revbuf, 8 * 1024, 0);
	closesocket(sockClient);
	//整理json结构
	CString strRecv;
	strRecv = revbuf;
...
int EditWhite(CString strIP, CString strSession, CString strID, CString strName, char*chJson, int jsonlen)
{
	USES_CONVERSION;
	strName = UrlEncode(strName);
	CString strPut = L"PUT /api/v1/facedb/white/" + strID + L" HTTP/1.1\n";
	CString strContent = L"name=" + strName;
	int iContentLen = strContent.GetLength();
	CString strSend;
	strSend.Format(L"%s%s%s%s%s%s%d%s%s%s%s%s%s%s%s",
...
int DeleteWhite(CString strIP, CString strSession, CString strID, char*chJson, int jsonlen)
{
	USES_CONVERSION;
	CString strSend;
	CString strDelete = L"DELETE /api/v1/facedb/white/" + strID + L" HTTP/1.1\n";
