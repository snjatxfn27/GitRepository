	...
	while (!pWnd->m_bQuit)
	{
		...
		while (!pWnd->m_bConnDisconnect)
		{
			...
			if (pWnd->m_bLogIn)
			{
				...
				if (nReturn != 0)
				{
					pWnd->PutInfoToList(_T("登录失败"));
					strBox.Format(L"获取Session失败，返回值为：%d", nReturn);
					pWnd->PutInfoToList(strBox);
					continue;
				}
				pWnd->PutInfoToList(_T("登录成功"));

				CString strSe;
				strSe = pWnd->chSession;
				strBox.Format(L"获取Session成功，Session为：%s", strSe);
				pWnd->PutInfoToList(strBox);
				pWnd->m_bLogIn = FALSE;
			}
			//从数据库中获取相似度
			...
			int re = 0;
			int iAge = 0;
			re = pWnd->ProcessOneSnapData(iSnapshotid, strFacePhotoName, strFullPhotoName, iAge);
			strBox.Format(L"处理完成，返回值为：%d", re);
			pWnd->PutInfoToList(strBox);
			if (re == 0)
			{
				pWnd->UpdateProcessStatusToDB(iSnapshotid, 3, iAge);
			}
			else if (re == 3)
			{
				pWnd->DeleteOneSnapshot(iSnapshotid, strFacePhotoName, strFullPhotoName);
			}
			else
			{
				pWnd->UpdateProcessStatusToDB(iSnapshotid, re + 10, iAge);
			}
		}
	}
	return 1;
...
int CDispatcherDlg::ProcessOneSnapData(int iSnapshotid, CString strFacePhotoName, CString strFullPhotoName, int &iAge)
{
	USES_CONVERSION;
	CString strBox;
	int iRe = 0;
	...
	else
	{
		...
		memset(chJson, 0, 1024 * 1024);
		CString strDBs(theApp.m_strTotalDBIDs);
		//比对白名单  W
		//比对黑名单  B
		//比对动态黑名单  TB
		int i(0);
		for (i = 0; i < 3; i++)
		{
			CString strDBID;
			if (i == 0)
				strDBID = theApp.m_strWID;
			else if (i == 1)
				strDBID = theApp.m_strBID;
			else if (i == 2)
				strDBID = theApp.m_strTBID;

			iRe = FASMatchPerson(T2A(theApp.m_strDetectServerIP), chSession, T2A(strDBID), theApp.m_dbTotalSim, T2A(strFeature), chJson, 1024 * 1024);
			if (iRe != 0)
			{
...
BOOL CDispatcherDlg::GetOneSnapshotFromDB(int &iSnapshotid, CString &strFacePhotoName, CString &strFullPhotoName)
{
	...
	pSet.CreateInstance(__uuidof(Recordset));
	try{
		CString strSQL = _T("select * from snapshot where imgisexist=1 and process_status=0 order by snapshotid limit 0,1");
		pSet = conn->Execute(_bstr_t(strSQL), NULL, adCmdText);
		if (pSet->adoEOF)
		{
		...
		iSnapshotid = pSet->GetCollect(_variant_t("Snapshotid")).dblVal;
		pSet->Close();
		pSet.Release();
...
BOOL CDispatcherDlg::UpdateProcessStatusToDB(int iSnapshotid, int iStatus, int iAge)
{
	if (!ConnectDB())
		return false;
	try{
		CString strSQL;
		strSQL.Format(L"UPDATE snapshot SET process_status=%d,age=%d WHERE snapshotid=%d", iStatus, iAge, iSnapshotid);
		conn->Execute(_bstr_t(strSQL), NULL, adCmdText);
	}
	catch (_com_error e)
	{
...
BOOL CDispatcherDlg::DeleteOneSnapshot(int iSnapshotid, CString strFacePhotoName, CString strFullPhotoName)
{
	if (!ConnectDB())
		return false;
	try{
		CString strSQL;
		strSQL.Format(L"DELETE FROM snapshot WHERE snapshotid=%d", iSnapshotid);
		conn->Execute(_bstr_t(strSQL), NULL, adCmdText);
	}
	catch (_com_error e)
	{
		AfxMessageBox(L"删除snapshot表一条数据失败");
		return false;
	}
	DeleteFileW(strFacePhotoName);
	DeleteFileW(strFullPhotoName);
	return true;
}
