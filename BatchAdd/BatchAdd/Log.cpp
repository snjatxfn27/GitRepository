...
static int DT_Log_Error_OpenFile(FILE** pf, char * pfileName)
{
	char	fileName[100];
	...
	sprintf(fileName, "%s\\%s", m_szDir, pfileName);
...
static void DT_Log_Error_Core(const char *file, int line, int level, int status, const char *fmt, va_list args,char * fileName)
{
  ...
  //  strLen += tmpStrLen;
    ...
    //加入LOG发生行数
   tmpStrLen = sprintf(str+strLen, " [line:%d]\n", line);
   strLen += tmpStrLen;
    ...
    //打开LOG文件
	if (DT_Log_Error_OpenFile(&pf, fileName))
	{
		return ;
	}
...
void DT_Log_Error(const char *file, int line, int level, int status, const char *fmt, char * pFileName...)
{
    va_list args;
if(level == DT_NO_LOG_LEVEL)
	...
	//调用核心的写LOG函数
	va_start(args, fmt);
	DT_Log_Error_Core(file, line, level, status, fmt, args, pFileName);
    va_end(args);
