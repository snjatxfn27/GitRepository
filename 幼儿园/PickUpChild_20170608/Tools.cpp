...
int CTools::DownloadImageFromFileServer(CString IP, int port, CString src, CString dst)
{
	...
	SOCKADDR_IN addrSrv;
	addrSrv.sin_addr.S_un.S_addr = inet_addr(T2A(IP.GetBuffer(0)));//该处地址应为空闲一体机地址
	...
	while (1)
	{
		int revlen = recv(sockClient, revbuf + iCurPos, 800 * 1024, 0);
	...
	CFile file;
	file.Open(L"http_response.txt", CFile::modeCreate | CFile::modeWrite);
